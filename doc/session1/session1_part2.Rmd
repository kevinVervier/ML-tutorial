...
# fix the randomness of the following lines, enable results reproducibility
set.seed(42)
...
* Idea: minimize a linear regression with additional constraints on the model
    ...
    * <b>Lasso</b> (top left): force some of the coefficients to be equal to 0: $\Omega(\beta) = |\beta|^2$,
    * <b>Ridge</b> (bottom): keep all coefficients in the same range, but no selection: $\Omega(\beta) = \|\beta\|_2^2$,

    * <b>Elastic-Net</b> (top right): combination of Ridge and Lasso, leading to sparse model and better handling correlated predictors $$\mbox{Elastic_Net}(\beta) = (1-\alpha)\times\mbox{Ridge}(\beta) + \alpha\times\mbox{Lasso}(\beta).$$
