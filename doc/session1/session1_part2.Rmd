...
# fit a model without cross-validation, just to see coefficient values evolution
fit.regul <- glmnet(x=as.matrix(obs[,-1]), y=obs[,1], alpha=best.alpha)
# Effect of lambda value on the model
par(mar=c(4.5,4.5,1,4))
plot(fit.regul,xvar = "lambda")
vnat=coef(fit.regul)
vnat=vnat[-1,ncol(vnat)] # remove the intercept, and get the coefficients at the end of the path
axis(2, at=vnat,line=-.5,label=names(vnat),las=1,tick=FALSE, cex.axis=0.5) 
