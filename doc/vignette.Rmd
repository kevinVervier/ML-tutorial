...
```{r }
data <- read.csv('../data/iris.csv',sep='\t')
head(data,10)
```
...
In the following sections, we will review standard classification algorithms with their own strategy/decision rule. For demonstration purpose, we will apply them to the `iris` data set in its standard use: ** Is it possible to predict to which species a new flower belongs ?**
...
```{r}
fit <- glmnet(x=as.matrix(data[,-1]), y=data[,1],family="binomial")
# Here, the predict function has a 'type' argument that defines the output: 'class' returns a class name and 'probability' 
```
...
* In this tutorial, we will use `LiblineaR` package:
```{r}
m <- LiblineaR(data=as.matrix(data[,-1]), target=data[,1],cost = 1, type = 2)
# you may want to optimize 2 parameters:
# cost determines the trade-off between the margin fit and the misclassification tolerance.
# Advanced: 'type' parameter determines the loss and regularization functions. you can for instance do logisti regression, or use a Lasso. 
